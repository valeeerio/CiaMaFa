...
- Onboarding minimale: accesso con Apple o Google (`signInWithIdToken`, nessuna email/password) + nickname univoco nel gruppo, niente foto profilo. Un profilo anonimo residuo (pre-Fase 10) si collega ad Apple/Google dal Profilo (`linkIdentityWithIdToken`) senza perdere nickname/piani/voti.
...
- Config (`SUPABASE_URL`, `SUPABASE_PUBLISHABLE_KEY`, opzionale `CARTO_API_KEY` per i tile Voyager, opzionali `FIREBASE_*` per le push e `GOOGLE_WEB_CLIENT_ID`/`GOOGLE_IOS_CLIENT_ID` per Google Sign-In: senza, rispettivamente niente notifiche di sistema e il pulsante Google non funziona) in `env.json` (gitignored, da `env.example.json`), letto come asset da `Env.load()`; `--dart-define` ha la precedenza. Mai committata.
