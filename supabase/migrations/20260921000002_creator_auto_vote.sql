-- Fase 5: il creatore del piano conta come "Ci sono" + Realtime sui voti.
-- (Stessa launch_plan della Fase 4, con il voto automatico dopo l'insert del piano.)
...
begin
  ...
  -- Chi propone ci va: voto "Ci sono" automatico (il trigger dei voti dà +0,5).
  insert into public.votes (plan_id, profile_id, vote) values (v_plan, v_uid, 'yes');
...
-- Realtime: conteggi e banner dei voti (RLS applicata: si vedono solo i voti
-- dei piani visibili).
do $$
begin
  if not exists (
