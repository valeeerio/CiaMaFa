...
- [ ] **Fase 12: Crea un nuovo gruppo**
  ...
  Fatto solo il backend (`supabase/migrations/20260924000001_create_group.sql`), volutamente senza UI: la schermata (probabilmente "entra nel gruppo esistente" vs "crea il tuo") tocca l'onboarding, su cui lavora in parallelo un'altra sessione. `create_group(nome)` (RPC): crea il gruppo, il creatore diventa owner in `group_members`, **non** cambia il tuo gruppo attivo (niente switcher prima della Fase 15) — resta `profiles.group_id` come oggi. `current_group_id()` aggiornata per restare deterministica quando un profilo appartiene a più gruppi contemporaneamente (vince `profiles.group_id` finché esiste). `groups_select` estesa: vedi anche i gruppi di cui sei owner/membro oltre a quello attivo (serve alla Fase 13). Da fare: UI + collegamento col flusso di onboarding.
